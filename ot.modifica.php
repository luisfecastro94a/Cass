...
<label for="">Asignar a Técnico</label>
...
<?php
include("conexion.php");
$consulta3="SELECT id_area, area FROM area";
...
<option value="<?php echo $reg['id_area']?>" selected=""><?php echo $reg['area']?></option>
 <?php
      while($fi=mysql_fetch_array($asig2))
  {?>
      <option value="<?php echo $fi['id_area']?>"><?php echo $fi['area'];?></option>
  <?php } ?>
</select>
</div>
...
<div class="col-xs-5" >
<label for="">ID</label>
<input type="text" class="form-control" name="id_cliente" id="id_cliente" maxlength="9"  
  required="" value="<?php echo $reg['id_cliente'];?>" ></div>
...
<div class="col-xs-5" >
          ...
          <input type="text" class="form-control" id="id_cotizacion" value="<?php echo $co['id_cotizacion'];?>" onKeyPress="return SoloNumeros(event)" name="correlativo_cotizacion" ></div>
...
<label class="fe" for="">Fecha Cotización<input class="" value="<?php echo $co['fecha_cotizacion'];?>" type="text" name="fecha_cotizacion" id="fecha_cotizacion"></label></div><!--fecha con jquey-->
 ...
 <div class="col-xs-5" >
  ...
  <div class="col-xs-5">
  <button type="submit" id="enviar" onclick="alert('¡Cambios realizados con Exito :) !!')" name="enviar" class="btn btn-primary btn-lg btn-block"><span class="glyphicon glyphicon-pencil"></span> Modificar Orden de Trabajo</button></div>
  <div class="col-xs-5">
  <button type="reset" class="btn btn-default btn-lg btn-block"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span> Cancelar</button>
  </div>
  <br>
  <br>
  <br>
...
include("conexion.php");
  ...
  $sql=mysql_query("UPDATE orden_trabajo SET id_usuario='$id_usuario', id_area='$id_area' WHERE id_orden_trabajo = '$id' ");
